...
- Tests (v3.0+): `core_tests/test_core.py` (pytest) covers pure logic (platform detection, regex, classifier, version compare, outtmpl/options, cancel hook, non-YouTube quality, 1440p/2160p, `verificar_url` cancel, prefs roundtrip). Run with `.venv/bin/python -m pytest`. Dev deps in `requirements-dev.txt`.
